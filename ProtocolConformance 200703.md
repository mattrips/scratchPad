...
The conformance of `X<Int>: P` has two implementations of the requirement *m1* of protocol `P`, implementations *i1* and *i2*.  While *i2* is constrained to cases in which requirement *m2* is satisfied with a type that conforms to protocol `Numeric`, that 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA0MTQyNDEwNCw1OTIwOTA1MDYsLTIwOD
