...
In the case of concretizations, under certain circumstances certain implementations are disregarded for purposes determining the conformance of the type to a protocol.  Specifically, given `T: P` and requirement *m* of `P`, if `T` is a concretization, any type has an implementation of a  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzI2OTI2ODc1LC0yMDg3MzM2MjkzLC0xMD
