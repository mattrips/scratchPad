...
If `t` is declared to conform to `p`, then, for protocol requirement `m` of `p`, one and only one of `t`'s possible implementations of `m` will be used to perform the protocol requirement.   which implementation is referred to as the *protocol witness* for the requirement. 
...
AnJ1xuIiwiaGlzdG9yeSI6Wy02MjE4NTk2MDgsMTc0MzUwOTky
