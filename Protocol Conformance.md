...
For a type `t` to conform to a protocol `p`, `t` must be declared to conform to `p`, and must have at least one unconditionally accessible implementation for each protocol requirement of `p`.  A declaration that `t` conforms to `p` further constitutes, with respect to each protocol `n` from which `p` directly or indirectly inherits, a declaration that `t` conforms to protocol `n` so long as the type has not already been declared to conform to protocol `n`.    
...
AnJ1xuIiwiaGlzdG9yeSI6Wy0zMjA3OTUwMTYsNTM3NTcyNTE2
