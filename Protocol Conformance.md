...
#### Protocol Witness
Given a protocol `p`, a protocol requirement is a statement in the declaration of `p` that describes a member `m`, such that, as a condition of a type `t` being able to conform to `p`, `t` must have a member satisfying the requirements of `m`.  A member of `t` that satisfies the requirements of `m`
...
AnJ1xuIiwiaGlzdG9yeSI6WzE0MjY2MDc0NzMsMTc0MzUwOTky
